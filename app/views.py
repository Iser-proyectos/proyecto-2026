...
def modelo_crear(request):
    ...
    Campos del formulario:
      ...
      Archivos:  archivo_modelo (.onnx), documento_estudio (PDF).
    ...
    Validaciones:
      ...
      - El archivo del modelo debe ser .onnx (validación de extensión).
    """
    ...
    if request.method == 'POST':
        ...
        if not (archivo_modelo.name.endswith('.onnx')):
            messages.error(request, 'Error: El archivo del modelo debe ser estrictamente formato .onnx')
            return render(request, 'modelo_form.html', {
                'categorias': ModeloML.CATEGORIA_CHOICES,
                'estados': ModeloML.ESTADO_CHOICES,
            })
...
def estadisticas(request):
    ...
    # 1. Totales Generales
    total_modelos = ModeloML.objects.count()
    total_ejecuciones = HistorialEjecucion.objects.count()
    
    # 2. Distribución de Modelos por Categoría
    # Ejemplo: [{'categoria': 'fresa', 'count': 5}, ...]
    categorias_qs = ModeloML.objects.values('categoria').annotate(count=Count('id'))
    categorias_nombres = [c['categoria'].capitalize() for c in categorias_qs]
    categorias_datos = [c['count'] for c in categorias_qs]
    
    # 3. Distribución de Ejecuciones por Categoría de Modelo (Porcentajes de Resultados)
    ejecuciones_qs = HistorialEjecucion.objects.values('modelo__categoria').annotate(count=Count('id'))
    
    ejec_nombres = []
    ejec_datos = []
    
    for e in ejecuciones_qs:
        cat = e['modelo__categoria']
        cat_name = cat.capitalize() if cat else 'Desconocida'
        ejec_nombres.append(cat_name)
        ejec_datos.append(e['count'])
        
    # Calcular porcentajes para mostrarlos en tarjetas
    ejecuciones_porcentajes = []
    if total_ejecuciones > 0:
        for idx, val in enumerate(ejec_datos):
            pct = round((val / total_ejecuciones) * 100, 1)
            ejecuciones_porcentajes.append({'nombre': ejec_nombres[idx], 'porcentaje': pct, 'cantidad': val})

    context = {
        'total_modelos': total_modelos,
        'total_ejecuciones': total_ejecuciones,
        'categorias_nombres_json': json.dumps(categorias_nombres),
        'categorias_datos_json': json.dumps(categorias_datos),
        'ejec_nombres_json': json.dumps(ejec_nombres),
        'ejec_datos_json': json.dumps(ejec_datos),
        'ejecuciones_porcentajes': ejecuciones_porcentajes,
    }
